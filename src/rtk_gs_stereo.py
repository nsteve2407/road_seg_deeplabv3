...
class Segmenter():
    def __init__(self,path_to_weights,publish_image=True,publish_tv=False):
    ...
    def pc_cb(self,pc_data,image_msg):
        ...
        op = np.squeeze(op)
        op_orig = np.expand_dims(op,axis=-1) 

        # Crop /Uncrop
        zero = np.zeros((size_init[0]-crop,size_init[1]),dtype=op.dtype)
        op_full = tf.expand_dims(op,axis=-1)
        op_full = tf.image.resize(images=op_full,size=[img.shape[0],img.shape[1]])
        op_full = np.squeeze(op_full)
        op_full = np.vstack([op_full,zero])
        ...
        # Publish
        pcd = np.copy(array)
        pcd['rgb'] = np.squeeze(op_rs.astype('<f4')) # Need to change dtpye?
        ...
        if self.publish_image:
            op = np.expand_dims(op,axis=-1)
            rgb_mask = self.decode_segmentation_masks(op_orig,[],self.num_classes)
            rgb_mask = tf.image.resize(images=rgb_mask, size=[crop, 672])
...
def main():
    ...
    pc_sub = msgf.Subscriber("/depth_registered/points",PointCloud2)
    img_sub = msgf.Subscriber("/zed/zed_node/left/image_rect_color",Image)

    ats = msgf.ApproximateTimeSynchronizer([pc_sub,img_sub],10,0.00001)

    ats.registerCallback(segmenter.pc_cb)

    # img_sub = rospy.Subscriber("/zed/zed_node/left/image_rect_color",Image,segmenter.img_cb)
